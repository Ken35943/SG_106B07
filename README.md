...
- **Data Provenance**: Fall-class training samples are **synthetic** (generated by `scripts/generate_synthetic_falls.py` using a 3-phase kinematic model for human subject safety). Walking/Sitting samples are from real physical recordings. **No real fall recordings exist yet** — every file under `data/raw/fall_*` (12 recordings) is synthetic (`sample_synth_*`); real cushioned fall capture on a safety mat is the next data-collection step.
